...
| `agents/` | 1,701 | **REWORK — keep the scaffolding, replace the agents** | `ToolBox`, structured Pydantic I/O, LangGraph wiring, `MockLLMClient`, fallback-on-failure discipline: all reusable and hard-won. The *specific* agents (Investigator, Recovery Strategy) are collections logic and go |
| `db/` | 302 | **KEEP + extend** | Org scoping and the session layer hold. Multi-tenancy has to widen from "one supplier org" to "supplier orgs **and** provider orgs", which is a real change to the tenancy model, not a config tweak |
...
| `api/` | 700 | **REWORK** | FastAPI patterns, schema conventions, camelCase serialisation all reusable. Every endpoint itself is supplier-side and gets replaced |
| `decision_engine/` | 1,271 | **REWORK — keep the shape** | Priority scoring, ranking, and the audit-trail construction are structurally right. The *decision* changes from "which track" to "which offer wins", and the approval gate stops being the centrepiece |
