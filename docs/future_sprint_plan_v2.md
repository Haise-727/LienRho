...
- The Python backend agents are powered by **NVIDIA LiteLLM** directly. We are officially deprecating all "NVIDIA LiteLLM" language across the repository, as NVIDIA LiteLLM was just a label, whereas LiteLLM is the actual implementation driving the multi-agent graph.
...
- **Task 1.3 [Verification Agent]:** Build an API route `/api/verify-document` that uses NVIDIA LiteLLM to extract entities from uploaded KYC documents, verifying both Sellers and Credit Providers against registered state data.

### Sprint 2: AI Capabilities & Communication (Next 4-8 Hours)
*Focus: Fulfilling the intelligent marketplace requirements.*

- **Task 2.1 [Chat UI]:** Build the Vercel AI SDK chat interface components, powered by NVIDIA LiteLLM.
