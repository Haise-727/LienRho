...
├── backend/              — FastAPI + Postgres + XGBoost + LangGraph, unchanged from source
...
|---|---|---|
| **Data model** | `canonical/`, `db/` | Add Provider/Offer/Opportunity/Match/Settlement models + migration; add party-type to auth/org model |
| **Provider & market data** | `data/synthetic.py` rework | Build the synthetic provider population (4–6 differentiated archetypes per `01-commerce-analysis.md` §6) — **with the same anti-leakage discipline as before**: provider pricing must not be generated by the same logic that later scores it |
| **Risk & utility** | `ml_core/`, cash forecast | Repurpose existing model output for pricing; build the Supplier Utility Engine from the existing forecast machinery |
| **Frontend shell** | `frontend/` | Strip the collections-shaped screens, stand up placeholder opportunity + provider views on the kept layout/auth/components |
